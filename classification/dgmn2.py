...
class DGMN2(nn.Module):
    def __init__(
        self, in_chans=3, num_classes=1000, embed_dims=(64, 128, 256, 512), num_heads=(1, 2, 4, 8),
...
@register_model
def dgmn2_tiny(pretrained=False, **kwargs):
    model = DGMN2(
...
@register_model
def dgmn2_small(pretrained=False, **kwargs):
    model = DGMN2(
        embed_dims=(64, 128, 320, 512), num_heads=(1, 2, 5, 8), mlp_ratios=(8, 8, 4, 4), qkv_bias=True,
...
@register_model
def dgmn2_medium(pretrained=False, **kwargs):
    model = DGMN2(
        embed_dims=(64, 128, 320, 512), num_heads=(1, 2, 5, 8), mlp_ratios=(8, 8, 4, 4), qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=(3, 4, 18, 3), **kwargs
    )
    model.default_cfg = _cfg()
...
@register_model
def dgmn2_large(pretrained=False, **kwargs):
    model = DGMN2(
        embed_dims=(64, 128, 320, 512), num_heads=(1, 2, 5, 8), mlp_ratios=(8, 8, 4, 4), qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=(3, 8, 27, 3), **kwargs
    )
    model.default_cfg = _cfg()
