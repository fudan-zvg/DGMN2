...
class DeformUnfoldFunction(Function):
    ...
    def forward(ctx,
        ...
        if not input.is_cuda:
            ...
            deform_conv_cuda.deform_unfold_forward_cuda(
                input, offset, output, ctx.bufs_[0],
    ...
    def backward(ctx, grad_output):
        ...
        if not grad_output.is_cuda:
            ...
            if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
                ...
                deform_conv_cuda.deform_unfold_backward_input_cuda(
                    input, offset, grad_output, grad_input,
                    grad_offset, ctx.bufs_[0], ctx.kernel_size[1],
                    ctx.kernel_size[0], ctx.stride[1], ctx.stride[0],
                    ctx.padding[1], ctx.padding[0], ctx.dilation[1],
                    ctx.dilation[0], ctx.deformable_groups,
                    cur_im2col_step)
